...
Issues.create = async (context) => {
  try {
    const { title, description, created_by } = context.request.body;
    ...
    await Revision.create({
      ...
      changes: {
        ...
        created_by: { from: null, to: issue.created_by }
      },
      created_by: created_by || 'unknown'
    });
...
Issues.update = async (context) => {
  try {
    ...
    if (updated_by !== undefined && updated_by !== originalValues.updated_by) {
      changes.updated_by = { from: originalValues.updated_by, to: updated_by };
    }
